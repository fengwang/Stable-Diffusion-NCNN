...
static inline ncnn::Mat randn_4_64_64( int seed )
{
...
static inline ncnn::Mat CFGDenoiser_CompVisDenoiser( ncnn::Net& net, float const* log_sigmas, ncnn::Mat& input, float sigma, ncnn::Mat cond, ncnn::Mat uncond )
{
    // get_scalings
...
inline static ncnn::Mat diffusion_solver( int seed, int step, ncnn::Mat& c, ncnn::Mat& uc )
{
    ncnn::Net net;
    {
        net.opt.use_vulkan_compute = false;
    ...
    // sample_euler_ancestral
    {
        for ( int i = 0; i < static_cast<int>(sigma.size()) - 1; i++ )
        {
            ...
            ncnn::Mat denoised = CFGDenoiser_CompVisDenoiser( net, log_sigmas, x_mat, sigma[i], c, uc );
...
inline static ncnn::Mat esr4x( ncnn::Mat& input )
{
    ncnn::Net net;
    {
        net.opt.use_vulkan_compute = false;
...
inline static ncnn::Mat prompt_solve( std::unordered_map<std::string, int>& tokenizer_token2idx, ncnn::Net& net, std::string prompt )
{
...
inline static std::pair<ncnn::Mat, ncnn::Mat> prompt_solver( std::string const& prompt_positive, std::string const& prompt_negative )
{
